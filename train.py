...
def train(adj, similarities, features):
    ...
    model = GCNModel(placeholders, num_features, features_nonzero, n, num_split)
    with tf.name_scope('optimizer'):
        opt = Optimizer(model.reconstructions, tf.sparse_tensor_to_dense(placeholders['similarities1'], validate_indices=False), tf.sparse_tensor_to_dense(placeholders['similarities2'], validate_indices=False), n)
    ...
    for epoch in range(FLAGS.epochs):
        t = time.time()
        feed_dict = construct_feed_dict(adj, similarities[0], similarities[1], features, placeholders)
        ...
        if all(x >= 0.5 for x in best_acc):
            break

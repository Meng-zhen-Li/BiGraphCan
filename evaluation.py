...
def LinkPrediction(embedding_look_up, edgelist_bipartite, testing_node):
    ...
    if testing_node <= np.max(edgelist_bipartite[:, 0]):
        test_neg_edges = np.append(np.ones((num_v - len(test_pos_edges), 1)) * testing_node, np.reshape(np.setdiff1d(np.unique(edgelist_bipartite[:, 1]), test_pos_edges[:, 1]), (-1, 1)), axis=1)
    else:
        test_neg_edges = np.append(np.reshape(np.setdiff1d(np.unique(edgelist_bipartite[:, 0]), test_pos_edges[:, 0]), (-1, 1)), np.ones((num_u - len(test_pos_edges), 1)) * testing_node, axis=1)

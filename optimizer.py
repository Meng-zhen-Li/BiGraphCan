...
class Optimizer(object):
    def __init__(self, preds, labels1, labels2, num_graphs):
        preds = [tf.split(x, num_graphs) for x in preds]
        labels1 = tf.split(labels1, num_graphs)
        labels2 = tf.split(labels2, num_graphs)

        self.optimizer = tf.train.AdamOptimizer(learning_rate=FLAGS.learning_rate)  # Adam Optimizer
        self.costs = []
        self.r2 = []
        self.weights = tf.zeros([num_graphs])
        ...
        for sim_idx in range(num_graphs):
            with tf.device(devices[sim_idx % len(devices)]):
                ...
                identifier = np.zeros(num_graphs)
                identifier[sim_idx] = 1
                identifier = tf.convert_to_tensor(identifier, dtype=tf.float32)
                self.weights = self.weights + (1 - R2) * identifier
        self.weights = self.weights / tf.sqrt(tf.reduce_sum(tf.square(self.weights)))
        ...
        for sim_idx in range(num_graphs):
            with tf.device(devices[sim_idx % len(devices)]):
                ...
                if self.grads_vars is None:
                    self.grads_vars = [(x[0] * self.weights[sim_idx], x[1]) for x in grads_var[:2]]
                    self.grads_vars.append(grads_var[2])
                else:
                    self.grads_vars[:2] = [(x[0] + y[0] * self.weights[sim_idx], x[1]) for x, y in zip(self.grads_vars[:2], grads_var[:2])]
